...
def exists(path, preload):
    ...
    if preload:
        #p = subprocess.Popen(["/usr/bin/eos", "root://eosuser.cern.ch/", "ls", path], stdout=subprocess.PIPE)
        p = subprocess.Popen(["/usr/bin/eos", "root://eosna62.cern.ch/", "ls", path], stdout=subprocess.PIPE)
        (out, _) = p.communicate();
        ret = p.returncode
        ...
        if os.path.dirname(path)==_preloaded[0] and (datetime.datetime.now()-_preloadTime)<datetime.timedelta(minutes=1):
            ...
            ret = subprocess.call(["/usr/bin/eos", "root://eosna62.cern.ch/", "ls", path])
    return ret==0

def mkDir(path):
    if not exists(path, False):
       #return subprocess.call(["/usr/bin/eos", "root://eosuser.cern.ch/", "mkdir", path])
       return subprocess.call(["/usr/bin/eos", "root://eosna62.cern.ch/", "mkdir", path])
